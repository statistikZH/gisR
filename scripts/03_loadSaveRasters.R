...
storage_pattern_list <- readRDS("extdata/tmp_storage_pattern.Rds")
...
Raster = terra::rast(single_files)
# terra::plotRGB(Raster, r=3, g=2, b=1, stretch="lin")

# mask and clip
# RGBI_zh <- st_crop(rasters, ktzh |> st_transform(st_crs(rasters)))
RGBI_zh <- terra::crop(Raster, ktzh_UTM32)
RGBI_zh <- terra::mask(RGBI_zh, ktzh_UTM32) |>
